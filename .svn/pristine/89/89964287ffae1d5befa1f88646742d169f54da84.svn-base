    ...
    render: function (template, params, linkTo, request, closeEvent) {
        ...
        _this.$api.getButtonType({
            ...
            table.render({
                ...
                url: config.InterfaceAddress.affairService + "/natureResource/sjbp/queryDkxxbById",
                where: {
                    workid: workid,
                    trackid: trackid
                },
            ...
            table.on('row(dk)', function (obj) {
                ...
                table.render({
                    ...
                    url: config.InterfaceAddress.affairService + "/natureResource/sjbp/queryDkzbbById",
                    method: 'post',
                    where: {
            ...
            upload.render({
                elem: '#dkImport'
                , url: config.InterfaceAddress.affairService + "/natureResource/sjbp/importExcelOrTxtDkxxbList"
                , accept: 'file'
                , data: {
                    workid: workid,
                    trackid: trackid,
            ...
            _this.$el.find("#0").click(function () {
                ...
                if (isImport) {
                    isImport = false;
                    location.href = config.InterfaceAddress.affairService + "/natureResource/sjbp/exportExcelOrTxtDkxxbList?workid=" + workid + "&fileType=0" + "&trackid=" + trackid;
                } else {
            ...
            _this.$el.find("#1").click(function () {
                ...
                if (isImport) {
                    isImport = false;
                    location.href = config.InterfaceAddress.affairService + "/natureResource/sjbp/exportExcelOrTxtDkxxbList?workid=" + workid + "&fileType=1" + "&trackid=" + trackid;
                } else {
                    layer.alert("导出失败,请先上传坐标并暂存再导出");
                }
            });
            ...
            _this.$el.find("#YDBJGJBZZBXGSMB").click(function () {
                var url = config.InterfaceAddress.affairService + "/natureResource/templates/用地报件国家标准坐标格式.zip";
