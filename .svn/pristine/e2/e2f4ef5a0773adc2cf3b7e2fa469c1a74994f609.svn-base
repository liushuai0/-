...
module.exports = {
    render: function (template, params, linkTo, request, closeEvent) {
        ...
        _this.$api.getButtonType({
            ...
            upload.render({
                elem: '#exprotExcel'
                , url: config.InterfaceAddress.affairService + "/natureResource/sjbp/importExcelOrTxtTdflmjbList"
                , accept: 'file'
                , data: {
                    workid: workid,
                    trackid: trackid
            ...
            _this.$el.find("#downloadTDFELJMB").click(function () {
                var url = config.InterfaceAddress.affairService + "/natureResource/templates/土地分类面积表模板.xls";
            ...
            table.render({
                ...
                url: config.InterfaceAddress.affairService + "/natureResource/sjbp/queryTdflmjbById"//模拟接口  如果开发中需要后台提供接口  直接将当前路径修改为接口路径   注意后台提供数据一定是json格式  格式必须按照json文件样例传递
                , method: "POST"
                , where: {
                    workid: workid,
                    trackid: trackid
